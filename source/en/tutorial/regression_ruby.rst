...
**rent.rb**
 ...
 1. Connect to Jubatus Server
  ...
  Connect to Jubatus Server (Line 28)
 ...
 2. Prepare the training data
  ...
  First, declare the variable of training data "trainDat", as a TupleFloatDatum List (Line 36).
  Next, read the source file (CSV file) of the training data line by line by foreach() method (Line 36-61).
  Split the data read from each line in CSV file, by the ',' mark (Line 44).

  The string items and double items are stored into the Datum components of string_values and num_values (Line 46-54), respectively. Then, a Datum class is generated by datum() method (Line 56). Finally, the Datum is appended with the rent label, so as to be used as one piece of training data (argument 'train' in Line 57).
  
  
 3. Model Training (update learning model

  Input the training data generated in step.2 into the train() method (Line 60).
 ...
 4. Prepare the prediction data 
  ...
  Read the YAML file (myhome.yml) by YAML.load_documents() and get the return value in hash type (Line 72).
  Generate the prediction Datum by using the simliar process as in step 2 (Line 73-82).
 ...
 5. Prediction by the regression model
  ...
  The prediction results are returned as a list by the estimate() method (Line 85).
 ...
 6. Output the result
  ...
  The result is rounded at 2nd decimal for output, because it is in Float type (Line 87).
